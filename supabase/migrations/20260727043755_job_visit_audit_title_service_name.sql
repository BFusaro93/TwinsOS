-- crm_job_visits audit titles were just "Job #1234 — 2026-07-27", so two
-- sibling visits of the same package job scheduled the same day (a common
-- case — crm_job_visits.job_service_id links a visit to one of the job's
-- several crm_job_services steps) produced identical, indistinguishable
-- titles in the Audit Trail. Join through job_service_id to append the
-- specific service name, e.g. "Job #1234 (Fert 2/5) — 2026-07-27".
--
-- fn_audit_log() is a large shared function used by many tables' triggers —
-- this is the exact live prod definition (fetched via pg_get_functiondef)
-- with only the crm_job_visits title CASE arm changed, per this repo's
-- established convention of never trusting local migration history for it.
