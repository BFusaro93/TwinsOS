-- Projects had no way to own an invoice. crm_invoices carried client_id,
-- estimate_id and crm_job_id but no project_id, so the Projects Billing tab
-- fell back to listing EVERY invoice for the project's client -- a client with
-- a $40k patio project and a $200/mo mowing route saw the mowing invoices
-- counted against the patio project's budget on the Analysis tab.
--
-- Add the direct link, and keep it populated automatically: a job-billed
-- invoice already knows its project through crm_jobs.project_id, so a trigger
-- derives project_id from crm_job_id whenever the caller didn't set one. That
-- covers the visit-completion auto-invoice path without touching it.

alter table public.crm_invoices
  add column if not exists project_id uuid references public.projects(id) on delete set null;

create index if not exists idx_crm_invoices_project
  on public.crm_invoices (org_id, project_id) where deleted_at is null;

-- Derive project_id from the job the invoice was billed against. Only fills a
-- NULL -- an explicitly-set project_id always wins, so invoicing a milestone
-- straight from a project isn't overwritten by an unrelated job link.
create or replace function public.fn_crm_invoices_derive_project_id()
returns trigger
language plpgsql
security definer
...
as $$
begin
  if new.project_id is null and new.crm_job_id is not null then
...
revoke execute on function public.fn_crm_invoices_derive_project_id() from public, anon;

drop trigger if exists trg_crm_invoices_derive_project_id on public.crm_invoices;
create trigger trg_crm_invoices_derive_project_id
  before insert or update of crm_job_id, project_id on public.crm_invoices
  for each row execute function public.fn_crm_invoices_derive_project_id();

-- Backfill history through the same job hop the WIP report was using.
...
-- rpt_projects_wip can now read the direct column instead of re-deriving the
-- job hop -- and it picks up project-attached invoices that have no job at all
-- (milestone billing), which the old join silently missed.
