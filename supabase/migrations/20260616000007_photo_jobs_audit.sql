...
BEGIN
  ...
  v_title := CASE TG_TABLE_NAME
    ...
    ELSE
      COALESCE(r_new->>'name', r_old->>'name', r_new->>'title', r_old->>'title', v_record_id::text)
  END;
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (v_org_id, v_user_id, v_record_type, v_record_id, 'created', v_user_name,
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF (r_old->>'deleted_at') IS NULL AND (r_new->>'deleted_at') IS NOT NULL THEN
      INSERT INTO public.audit_log (org_id, created_by, record_type, record_id, action, changed_by_name, description)
      VALUES (v_org_id, v_user_id, v_record_type, v_record_id, 'deleted', v_user_name,
    ...
    THEN
      ...
      VALUES (v_org_id, v_user_id, v_record_type, v_record_id, 'qty_adjusted', v_user_name,
        v_title || ': quantity adjusted', 'quantity_on_hand',
        r_old->>'quantity_on_hand', r_new->>'quantity_on_hand');

    ELSIF (r_old->>'status') IS DISTINCT FROM (r_new->>'status') AND (r_old->>'status') IS NOT NULL THEN
      INSERT INTO public.audit_log (org_id, created_by, record_type, record_id, action,
        changed_by_name, description, field_changed, old_value, new_value)
      VALUES (v_org_id, v_user_id, v_record_type, v_record_id, 'status_changed', v_user_name,
        v_title || ' status changed', 'status', r_old->>'status', r_new->>'status');

    ELSE
      v_fields := CASE TG_TABLE_NAME
        WHEN 'vehicles'        THEN ARRAY['name','asset_tag','equipment_number','make','model','year','vin','license_plate','fuel_type','engine_model','division','assigned_crew','location','barcode','purchase_vendor_name','purchase_date','payment_method','finance_institution','next_oil_change_due','next_oil_change_mileage','next_inspection_sticker_due','notes','air_filter_part_number','oil_filter_part_number','spark_plug_part_number','photo_url']
        WHEN 'assets'          THEN ARRAY['name','asset_tag','equipment_number','make','model','year','serial_number','engine_serial_number','engine_model','manufacturer','division','assigned_crew','location','barcode','purchase_vendor_name','purchase_date','payment_method','finance_institution','notes','air_filter_part_number','oil_filter_part_number','spark_plug_part_number','photo_url']
        WHEN 'parts'           THEN ARRAY['name','part_number','category','description','unit_cost','minimum_stock','vendor_name','vendor_id','is_inventory','notes','picture_url','alternate_vendors']
        WHEN 'product_items'   THEN ARRAY['name','part_number','category','description','unit_cost','price','vendor_name','vendor_id','is_inventory','picture_url','alternate_vendors']
        WHEN 'work_orders'     THEN ARRAY['title','priority','assigned_to_name','assigned_to_names','due_date','category','categories','description','wo_type']
        WHEN 'requisitions'    THEN ARRAY['title','vendor_name','notes','grand_total']
        WHEN 'purchase_orders' THEN ARRAY['vendor_name','notes','grand_total','invoice_number','po_date']
        WHEN 'vendors'         THEN ARRAY['name','contact_name','email','phone','address','vendor_type','is_active','notes','website']
        WHEN 'projects'        THEN ARRAY['name','customer_name','address','start_date','end_date','notes']
        WHEN 'pm_schedules'    THEN ARRAY['title','frequency','next_due_date','last_completed_date','is_active','description']
        WHEN 'goods_receipts'  THEN ARRAY['receipt_number','vendor_name','po_number','received_by_name','notes','subtotal','grand_total']
        WHEN 'damage_cases'    THEN ARRAY['customer_name','property_address','date_of_incident','description','resolution_notes','purchase_order_id']
        WHEN 'photo_jobs'      THEN ARRAY['name','address','notes','status','project_id']
      ...
      FOREACH v_field IN ARRAY v_fields LOOP
        ...
        THEN
          ...
          INSERT INTO public.audit_log (org_id, created_by, record_type, record_id, action,
            changed_by_name, description, field_changed, old_value, new_value)
          VALUES (v_org_id, v_user_id, v_record_type, v_record_id, 'updated', v_user_name,
            initcap(replace(v_field, '_', ' ')) || ' updated', v_field,
            r_old->>v_field, r_new->>v_field);
        END IF;
      END LOOP;
...
-- Attach to photo_jobs
CREATE TRIGGER trg_photo_jobs_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.photo_jobs
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

-- Backfill existing photo jobs
...
WHERE pj.deleted_at IS NULL
  AND NOT EXISTS (
    ...
    WHERE al.record_type = 'job_photo' AND al.record_id = pj.id
  );
