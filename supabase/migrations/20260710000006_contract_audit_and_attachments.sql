-- Wire up audit trail + attachments for CRM Contracts.
--
-- 1. fn_audit_log() never had a case for crm_contracts, so contract
--    create/update/delete produced no audit_log rows at all (record_type
--    fell through to the literal table name 'crm_contracts', which nothing
--    queries for). Contracts already have a `title` column, so the existing
--    generic v_title/field-diff logic in the ELSE branches works for them
--    unchanged — the only missing piece was the record_type mapping.
-- 2. No trigger was ever attached to crm_contracts, so even with the mapping
--    fixed, nothing would fire the function on contract changes.
-- 3. attachments_record_type_check never included 'contract' — and, while
--    touching this constraint, also never included 'ticket' or 'damage_case'
--    despite AttachmentsSection already being used with both record types
--    (TicketDetailSheet, DamageCaseDetailPanel). Uploading an attachment for
--    a ticket or damage case currently fails with a constraint violation;
--    fixed here alongside the contract addition since it's the same column.

-- ── fn_audit_log(): map crm_contracts -> 'contract' ─────────────────────────
-- (Reproduces the function currently live in prod verbatim, plus the one new
-- WHEN arm, since local migration history had drifted from what's deployed.)
...
-- ── attach the trigger to crm_contracts ─────────────────────────────────────

DROP TRIGGER IF EXISTS trg_crm_contracts_audit ON public.crm_contracts;
CREATE TRIGGER trg_crm_contracts_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.crm_contracts
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

-- ── attachments: add 'contract', and the already-missing 'ticket' / 'damage_case' ──
