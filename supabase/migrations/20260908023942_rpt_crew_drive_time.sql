-- Report Center support for reviewing recorded crew drive time
-- (crm_crew_drive_segments) — day-level, crew-scoped, not per-visit.
create or replace view rpt_crew_drive_time with (security_invoker = on) as
select
  s.id,
  s.work_date,
  ...
  s.ended_at,
  s.minutes
from crm_crew_drive_segments s
left join crm_crews cw on cw.id = s.crew_id;

-- Whitelist rpt_crew_drive_time on crm_run_report — everything else in this
-- function is unchanged from the live definition of 2026-09-06 (NY-tz +
-- total_count version).
...
declare
  v_allowed_datasets constant text[] := array[
    ...
    'rpt_contract_service_usage', 'rpt_crew_drive_time'
  ];
