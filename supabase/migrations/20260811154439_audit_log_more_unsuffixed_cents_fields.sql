-- Follow-up to 20260811000000_audit_log_dollar_format_fix.sql: that migration
-- only widened the dollar-format check to unit_cost/price. Turns out the same
-- "unsuffixed cents column" problem exists on several other tables that were
-- still showing raw integers in their audit trail:
--   - purchase_orders / requisitions / goods_receipts: subtotal, sales_tax,
--     shipping_cost, grand_total, discount_cost — this made almost every PO
--     edit show up as e.g. "subtotal: 7799 -> 10998" instead of dollars.
--   - projects: contract_price, total_cost
--   - assets / vehicles: purchase_price
--   - wo_vendor_charges: cost (only reachable via its UPDATE path — INSERT/
--     DELETE already had their own dollar-formatted cost note)
--
-- Rebuilt from prod's live pg_get_functiondef('public.fn_audit_log()') per
-- the drift gotcha in project_supabase_environments memory — do not base
-- this off any older migration file's copy of the function body.
