-- Two bugs found while wiring up required-reason manual quantity adjustments
-- for Products (mirroring the Parts feature in 20260814000001):
--
-- 1. adjust_product_item_quantity() (20260808000000) inserted its audit row
--    with record_type = 'product_item', but every other writer for this
--    table (fn_audit_log()'s generic branch, and the AuditTrailTab UI query)
--    uses 'product'. The RPC's attributed, reason-carrying row was silently
--    invisible everywhere in the app — every manual adjustment appeared to
--    only produce the generic reason-less "qty X -> Y" entry.
--
-- 2. fn_audit_log() has a suppression check (app.suppress_parts_qty_audit)
--    for the `parts` table's generic qty_adjusted branch, but never got the
--    equivalent for `product_items` — so even with (1) fixed, every RPC call
--    would still produce a duplicate generic row alongside the real one.
