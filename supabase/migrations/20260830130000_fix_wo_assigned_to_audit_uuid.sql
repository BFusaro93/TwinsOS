-- The generic changed-fields diff in fn_audit_log() falls through to raw
-- jsonb text for any column not explicitly skipped/special-cased. When
-- work_orders.assigned_to_id / crm_tickets.assigned_to_id was repointed from
-- profiles(id) to crm_employees(id) (20260825134105), it stayed out of both
-- the skip list and the special-case branches, so activity log entries show
-- "assigned to id: <uuid> -> <uuid>" instead of a technician name. Add a
-- dedicated branch (same pattern as the discount-fields branch) that
-- resolves both ids to crm_employees names, and skip the raw id in the
-- generic loop so it isn't logged twice.
...
declare
  ...
  v_asgn_old_id   uuid;
  v_asgn_new_id   uuid;
  v_asgn_old_name text;
  v_asgn_new_name text;
begin
  ...
  if TG_OP = 'INSERT' then
    ...
    then
      ...
      if TG_TABLE_NAME in ('work_orders', 'crm_tickets')
        and (r_old ->> 'assigned_to_id') is distinct from (r_new ->> 'assigned_to_id')
      then
        v_asgn_old_id   := (r_old ->> 'assigned_to_id')::uuid;
        v_asgn_new_id   := (r_new ->> 'assigned_to_id')::uuid;
        v_asgn_old_name := null;
        v_asgn_new_name := null;
        if v_asgn_old_id is not null then
          select nullif(trim(coalesce(first_name, '') || ' ' || coalesce(last_name, '')), '')
          into v_asgn_old_name
          from crm_employees where id = v_asgn_old_id;
        end if;
        if v_asgn_new_id is not null then
          select nullif(trim(coalesce(first_name, '') || ' ' || coalesce(last_name, '')), '')
          into v_asgn_new_name
          from crm_employees where id = v_asgn_new_id;
        end if;
        v_changed_parts := v_changed_parts ||
          ('assigned to: ' || coalesce(v_asgn_old_name, 'unassigned') || ' → ' || coalesce(v_asgn_new_name, 'unassigned'));
      end if;
      ...
      v_skip_keys := array[
        ...
        'discount_cents','discount_type','discount_value','applied_discount_id',
        'cost_layers','quantity_on_hand','assigned_to_id'
      ];
