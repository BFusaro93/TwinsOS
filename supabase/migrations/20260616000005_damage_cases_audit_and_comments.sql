-- ─────────────────────────────────────────────────────────────────────────────
-- Wire damage_cases into the existing fn_audit_log trigger infrastructure
-- and extend fn_audit_log to handle the damage_case record type.
-- ─────────────────────────────────────────────────────────────────────────────

-- 1. Update fn_audit_log to recognise damage_cases
...
BEGIN
  ...
  v_title := CASE TG_TABLE_NAME
    ...
    WHEN 'damage_cases'    THEN
      COALESCE(r_new ->> 'case_number', r_old ->> 'case_number', '')
    WHEN 'vehicles'        THEN
      COALESCE(
        NULLIF(TRIM(
          COALESCE(r_new ->> 'year', '') || ' ' ||
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    ELSE
      v_fields := CASE TG_TABLE_NAME
        ...
        WHEN 'damage_cases' THEN ARRAY[
          'customer_name', 'property_address', 'date_of_incident',
          'description', 'resolution_notes', 'purchase_order_id'
        ]
...
-- 2. Attach the trigger to damage_cases
CREATE TRIGGER trg_damage_cases_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.damage_cases
