-- Allow aggregate queries with no GROUP BY columns (a single grand-total
-- row over the whole filtered set) — needed for KPI dashboard visuals like
-- "Total Invoiced This Month". Previously the aggregate branch only
-- triggered when p_group_by had entries, so "Sum of X with zero group
-- columns" fell through to the plain-columns branch and was rejected.
-- Postgres itself allows aggregate SELECTs with no GROUP BY (the whole
-- result set is treated as one group); the SQL construction below already
-- handles an empty v_group_parts correctly, so only the entry condition
-- needs to change.
