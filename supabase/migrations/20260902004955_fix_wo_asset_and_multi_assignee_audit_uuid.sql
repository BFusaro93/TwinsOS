-- 20260830130000 fixed work_orders/crm_tickets.assigned_to_id (singular)
-- leaking a raw uuid into the audit trail, via a dedicated lookup branch
-- that resolves it to a crm_employees name. But work_orders also has a
-- newer PLURAL assigned_to_ids (multi-assignee) column that fix never
-- touched, and asset_id was never covered by any fix at all — both still
-- fall through the generic per-field diff loop and print raw ids/arrays
-- (confirmed live on WO-2026-000617: "asset id: blank -> <uuid>",
-- "assigned to ids: [] -> [\"<uuid>\"]").
--
-- work_orders already denormalizes asset_name/assigned_to_name/
-- assigned_to_names alongside the id columns (populated client-side by
-- NewWorkOrderDialog), so the fix is simply to skip the raw id columns in
-- the generic loop — the parallel *_name column already produces an
-- equivalent, readable line. This also makes the dedicated assigned_to_id
-- lookup branch redundant (it duplicated what assigned_to_name already
-- shows — visible in the same audit entry as two near-identical lines,
-- "assigned to: X" and "assigned to name: X"), so it's removed rather than
-- extended to cover the plural column too.
--
-- Confirmed no other table with an asset_id column (asset_parts,
-- maintenance_requests, meters, pm_schedule_assets, pm_schedules) lacks a
-- parallel asset_name EXCEPT asset_parts, which has no fn_audit_log()
-- trigger attached at all — so skipping asset_id globally is safe.
  ...
  if TG_OP = 'INSERT' then
    ...
    then
      ...
      v_skip_keys := array[
        ...
        'assigned_to_id','assigned_to_ids','asset_id'
      ];
