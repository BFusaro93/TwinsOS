-- receive_part_quantity was overwriting product_items.price (the
-- customer-facing sale price, which may include a markup over cost) with
-- the raw receiving cost on every goods receipt, destroying any markup the
-- user had configured. Only unit_cost should track the receiving cost;
-- price is a separate, user-managed field.
