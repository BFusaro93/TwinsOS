-- Clients already had a `client` AuditRecordType and a wired-up Audit Trail tab in the
-- UI, but fn_audit_log() was never actually attached to the `clients` table as a trigger,
-- so the tab always rendered empty. Services and Packages get the trigger too (their
-- UI tabs are added in this same change) — fn_audit_log()'s generic name/title/display_name
-- fallback and org_id/deleted_at handling already work for both tables unmodified, so this
-- only needs to add clean record_type labels ('service', 'package' instead of the raw
-- table name) to the existing CASE mapping.
--
-- Based on the exact live function body (fetched via pg_get_functiondef immediately
-- before writing this migration) plus the two new CASE branches — see the "shared
-- trigger function bodies" gotcha in project memory before ever replacing this function.
...
drop trigger if exists trg_crm_services_audit on crm_services;
create trigger trg_crm_services_audit
  after insert or delete or update on crm_services
  for each row execute function fn_audit_log();

drop trigger if exists trg_crm_packages_audit on crm_packages;
create trigger trg_crm_packages_audit
  after insert or delete or update on crm_packages
  for each row execute function fn_audit_log();
