-- Start/End on a visit are meant to represent the ACTUAL job time, not a
-- schedule — crew clock-in/out punches often need dispatcher correction, so
-- treating start_time/end_time and clocked_in_at/clocked_out_at as two
-- separate, unsynced facts left dispatchers with no visibility into real
-- clock times at all. Rather than dropping one pair (clocked_in_at/out is
-- still needed as-is by the crew tablet's own elapsed-timer UI and by
-- date-range filtering in Job Costing/COGS/Report Center, none of which a
-- time-only column can support), both write paths (app code, in this same
-- round) now keep both pairs in sync — this migration only handles the
-- one-time backfill of existing rows plus wiring up an audit trail so future
-- corrections are visible.
--
-- Backfill: existing completed visits that only ever got a clock-in/out (no
-- start_time/end_time yet) should show correctly once the dispatch board
-- starts reading these as the same fact. All client data seen this session
-- is Worcester/Sutton/Millbury, MA, so America/New_York is used for this
-- one-time conversion — adjust if the org isn't actually Eastern time.
update crm_job_visits
set start_time = (clocked_in_at at time zone 'America/New_York')::time
where start_time is null and clocked_in_at is not null;

update crm_job_visits
set end_time = (clocked_out_at at time zone 'America/New_York')::time
where end_time is null and clocked_out_at is not null;

-- Audit trail: crm_job_visits had zero trigger coverage. Re-create
-- fn_audit_log() from its current live definition (this function is known
-- to drift from local migration history — see 20260717000008's own comment
-- — so this was fetched via pg_get_functiondef() on prod just before writing
-- this migration) with two additions: a record_type arm and a title arm for
-- crm_job_visits, so visit edits (start_time, end_time, crew_id, men_count,
-- status, etc.) start showing up in the generic audit_log the same way
-- crm_jobs/clients/invoices already do.
...
begin
  ...
  v_title := case TG_TABLE_NAME
    ...
    when 'crm_job_visits'  then coalesce(
      (select 'Job #' || j.job_number from crm_jobs j
...
drop trigger if exists trg_crm_job_visits_audit on crm_job_visits;
CREATE TRIGGER trg_crm_job_visits_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.crm_job_visits
