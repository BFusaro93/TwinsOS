-- Wire up audit trail for Work Order costs: parts, labor, and vendor charges.
--
-- None of wo_parts / wo_labor_entries / wo_vendor_charges ever had an audit
-- trigger attached — only trg_*_updated_at (timestamp bookkeeping). Adding a
-- part (with quantity + unit cost), a labor entry, or a vendor charge to a
-- Work Order produced zero audit_log rows for the WO's own Audit Trail tab.
-- The only audit signal was the *inventory* side effect: adjust_part_quantity()
-- writes a 'part' record_type row for the catalog part's quantity change, but
-- that's keyed to the part's id, not the work order's, so it never shows up
-- on the WO's audit trail either.
--
-- Fix mirrors the existing crm_invoice_line_items/estimate_line_items
-- special-case already in fn_audit_log(): child cost rows are attributed to
-- their PARENT record (record_type='work_order', record_id=work_order_id) so
-- they surface directly in the WO's own Audit Trail tab with no new UI/type
-- needed. Create/delete descriptions include the qty/rate/cost so "parts and
-- costs" are visible at a glance, not just "a part was added".
--
-- Reproduces the function currently live in prod verbatim (confirmed via
-- pg_get_functiondef on prod, per the standing rule that local migration
-- history has drifted from what's deployed) plus the new WO-costs branch.
...
begin
  ...
  if TG_TABLE_NAME in ('crm_invoice_line_items', 'estimate_line_items', 'wo_parts', 'wo_labor_entries', 'wo_vendor_charges') then
    ...
    -- cost detail suffix on create/delete so "parts and costs" are visible at a glance
    ...
    if TG_OP = 'INSERT' then
      ...
      v_description := case TG_TABLE_NAME
        when 'crm_invoice_line_items' then 'Line item added: ' || v_title
        when 'estimate_line_items'    then 'Line item added: ' || v_title
      ...
      v_action      := 'deleted';
      v_description := case TG_TABLE_NAME
        when 'crm_invoice_line_items' then 'Line item removed: ' || v_title
        when 'estimate_line_items'    then 'Line item removed: ' || v_title
...
-- ── attach triggers to the three WO cost tables ─────────────────────────────

DROP TRIGGER IF EXISTS trg_wo_parts_audit ON public.wo_parts;
CREATE TRIGGER trg_wo_parts_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.wo_parts
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

DROP TRIGGER IF EXISTS trg_wo_labor_entries_audit ON public.wo_labor_entries;
CREATE TRIGGER trg_wo_labor_entries_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.wo_labor_entries
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

DROP TRIGGER IF EXISTS trg_wo_vendor_charges_audit ON public.wo_vendor_charges;
CREATE TRIGGER trg_wo_vendor_charges_audit
  AFTER INSERT OR UPDATE OR DELETE ON public.wo_vendor_charges
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();
