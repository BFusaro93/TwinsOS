-- Follow-up to 20260901030000/20260901040000. Those fixed raw-uuid leaks,
-- but WO-2026-000617's audit trail still showed the assignee THREE times:
-- "assigned to: unassigned -> Casey Kleinman; assigned to name: blank ->
-- Casey Kleinman; assigned to names: [] -> [\"Casey Kleinman\"]".
--
-- Root cause: assigned_to_id/assigned_to_name (singular) are pure mirrors
-- of assigned_to_ids/assigned_to_names[0] (plural) -- NewWorkOrderDialog.tsx
-- sets `assignedToId: assignedToIds[0]` and its name together, every save,
-- unconditionally (see commonFields). They carry zero unique information.
-- assigned_to_id was already skipped in the audit trigger; assigned_to_name
-- was not, so it still produced a redundant line. This skips it too,
-- leaving only assigned_to_names (the current multi-assignee source of
-- truth) visible going forward.
--
-- Also backfills the ~7 existing historical rows where multiple
-- "assigned to[...]:" phrasings already coexisted (from the pre-fix
-- dedicated trigger branch removed in 20260901030000, plus the raw-id
-- backfill in 20260901040000 which intentionally rewrote unreadable
-- segments but didn't yet collapse rows that already had a readable
-- alternative) -- collapses down to one canonical phrasing per row,
-- preferring plural names > singular name > the plain "assigned to:" text.
...
begin
      ...
      if (r_old ->> 'deleted_at') is null and (r_new ->> 'deleted_at') is not null then
        ...
        for v_key in select jsonb_object_keys(r_new) loop
          ...
          if (r_old ->> v_key) is distinct from (r_new ->> v_key) then
            if v_key like '%\_cents' or v_key in ('unit_cost', 'price', 'cost') then
              ...
              v_label     := replace(regexp_replace(v_key, '_cents$', ''), '_', ' ');
            else
            ...
            v_changed_parts := v_changed_parts ||
              (v_label || ': ' || v_old_field || ' → ' || v_new_field);
          end if;
        end loop;
  ...
  if TG_OP = 'INSERT' then
    ...
    elsif TG_TABLE_NAME in ('parts', 'product_items')
    ...
    then
      ...
      for v_key in select jsonb_object_keys(r_new) loop
        continue when v_key = any(v_skip_keys);
        if (r_old ->> v_key) is distinct from (r_new ->> v_key) then
          if v_key like '%\_cents' or v_key in (
...
-- Backfill: collapse rows where multiple "assigned to[...]:" phrasings
-- already coexist into a single canonical one (prefer plural names >
-- singular name > plain "assigned to:").
DO $$
DECLARE
  r RECORD;
  segs text[];
  ...
  new_segs text[];
  prefix text;
  has_names_plural boolean;
  has_name_singular boolean;
  kept_one boolean;
BEGIN
  FOR r IN
    ...
    WHERE record_type = 'work_order'
      AND (
        (description ~ 'assigned to name: ' AND description ~ 'assigned to names: ')
        OR (description ~ '(^|; )assigned to: ' AND description ~ 'assigned to name')
      )
  LOOP
    prefix := split_part(r.description, ' — ', 1);
    segs := string_to_array(substring(r.description from position(' — ' in r.description) + 3), '; ');
    has_names_plural := r.description ~ 'assigned to names: ';
    has_name_singular := r.description ~ 'assigned to name: ';
    new_segs := ARRAY[]::text[];
    kept_one := false;

    FOREACH seg IN ARRAY segs LOOP
      IF seg LIKE 'assigned to: %' OR seg LIKE 'assigned to name: %' OR seg LIKE 'assigned to names: %' THEN
        IF has_names_plural THEN
          IF seg LIKE 'assigned to names: %' THEN new_segs := new_segs || seg; END IF;
        ELSIF has_name_singular THEN
          IF seg LIKE 'assigned to name: %' THEN new_segs := new_segs || seg; END IF;
        ELSE
          IF NOT kept_one THEN new_segs := new_segs || seg; kept_one := true; END IF;
        END IF;
      ELSE
    ...
    UPDATE audit_log SET description = prefix || ' — ' || array_to_string(new_segs, '; ') WHERE id = r.id;
  END LOOP;
END $$;
