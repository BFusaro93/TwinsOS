-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: audit_field_level_tracking
-- Replaces fn_audit_log() with a version that writes one audit_log row per
-- changed field for generic UPDATE operations, instead of a single generic
-- "record updated" row. This gives users a clear change history showing
-- exactly what was changed, by whom, and from/to what value.
--
-- Status changes and quantity adjustments are still written as single rows
-- (they already carry old/new values).
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  -- Represent OLD and NEW as jsonb for generic column access.
  ...
  -- Map table name → audit record_type label (matches TypeScript AuditRecordType)
  ...
  -- Resolve current user name (falls back to 'System' for service-role sessions)
  v_user_id := auth.uid();
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- 1. Soft-delete: deleted_at NULL → non-NULL
    ...
    -- 4. Field-level tracking for all other updates
    ELSE
      -- Per-table list of "interesting" fields to diff
      v_fields := CASE TG_TABLE_NAME
        ...
        WHEN 'parts' THEN ARRAY[
          ...
          'unit_cost', 'minimum_stock', 'vendor_name', 'notes', 'picture_url'
        ]
        WHEN 'product_items' THEN ARRAY[
          'name', 'part_number', 'category', 'description',
          'unit_cost', 'price', 'vendor_name', 'picture_url'
        ]
      ...
      -- Fallback: if no tracked field changed (e.g., only updated_at bumped)
