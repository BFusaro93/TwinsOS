-- Real root cause of the duplicate audit rows (confirmed present for the
-- `parts` table too, going all the way back to 2026-07-14 -- not something
-- introduced recently): when app.suppress_parts_qty_audit is set, the
-- qty_adjusted elsif branch's condition becomes false, so execution falls
-- through the rest of the elsif chain into the generic catch-all branch at
-- the bottom -- which loops over every changed column and finds
-- quantity_on_hand changed (it was never in that loop's skip list), logging
-- a generic "Updated -- quantity on hand: X -> Y" entry anyway. Suppression
-- only ever skipped the *specific* branch, never the trigger's fallback
-- logging. Fix: return immediately when suppressed instead of falling
-- through.
