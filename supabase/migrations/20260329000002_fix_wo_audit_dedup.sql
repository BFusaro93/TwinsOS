-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: fix_wo_audit_dedup
-- Removes singular fields (assigned_to_name, category) from the work_orders
-- tracked fields list since the array versions (assigned_to_names, categories)
-- are already tracked. This prevents duplicate audit entries when clearing
-- all assignees or categories.
-- ─────────────────────────────────────────────────────────────────────────────
    ...
    ELSE
      v_fields := CASE TG_TABLE_NAME
        ...
        WHEN 'work_orders' THEN ARRAY[
          'title', 'priority', 'assigned_to_names',
          'due_date', 'categories', 'description', 'wo_type'
        ]
