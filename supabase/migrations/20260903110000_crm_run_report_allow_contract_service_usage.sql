-- Re-whitelist crm_run_report's allowed datasets to include rpt_contract_service_usage.
-- The view was created by 20260829130000_rpt_contract_service_usage.sql but the RPC
-- whitelist was never updated (and 20260829161000 was written from a stale copy), so
-- the Contract Service Usage report raised "Unknown dataset" on PROD.
-- Function body below is the LIVE prod definition (pg_get_functiondef, 2026-09-03)
-- with only the whitelist array changed.
CREATE OR REPLACE FUNCTION public.crm_run_report(p_dataset text, p_columns text[], p_filters jsonb DEFAULT '[]'::jsonb, p_group_by text[] DEFAULT NULL::text[], p_aggregates jsonb DEFAULT '[]'::jsonb, p_sort_column text DEFAULT NULL::text, p_sort_dir text DEFAULT 'asc'::text, p_limit integer DEFAULT 1000)
 RETURNS jsonb
 LANGUAGE plpgsql
