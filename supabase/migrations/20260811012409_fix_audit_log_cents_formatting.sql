-- fn_audit_log()'s generic field-diff loops label every changed column by
-- humanizing its raw name (replace(v_key, '_', ' ')), so any `*_cents`
-- column keeps a "cents" label in the audit description. The line-item
-- loop special-cased `rate_cents`/`total_cents` to format the value as a
-- dollar string but never adjusted the label, producing entries like
-- "rate cents: $0.00 -> $125.00". The top-level estimates/invoices loop
-- never got a dollar-formatting branch at all, producing entries like
-- "revenue cents: 0 -> 12500" (raw integer cents, still labeled "cents").
--
-- Fix: for any key ending in `_cents`, format both old/new as a dollar
-- string and strip the `_cents` suffix from the label before humanizing.
-- Generalized to all `*_cents` columns (not just rate_cents/total_cents)
-- so this covers revenue_cents, net_profit_cents, gross_profit_cents, etc.
--
