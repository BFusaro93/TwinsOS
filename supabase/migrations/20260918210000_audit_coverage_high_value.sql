-- ─────────────────────────────────────────────────────────────────────────────
-- Audit coverage: approvals, identity, credentials, and the remaining
-- money/config tables in both products.
--
-- Extends the engine from 20260918200000 rather than replacing its ideas:
--
--  * Approval decisions (`approval_requests`) now land on the REQUISITION,
--    PO or ESTIMATE being approved, phrased as a decision rather than a
--    field diff — "Approval step 2 approved by Dana — 'ok to order'". The
--    flow definition itself (`approval_flows` / `approval_flow_steps`) is
--    audited too, so changing who can approve is on the record.
--
--  * `profiles` is audited, and `org_id` is deliberately NOT skipped there:
--    a profile moving between orgs is the exact shape of the cross-org
--    escalation already found in this codebase. The entry is written under
--    the ORIGINAL org so the org the user left can still see it.
--
--  * Credentials (`api_keys`, `integrations`, `oauth_tokens`) are audited
--    with secret values redacted — audit_log is readable by every member of
--    the org, so hashes, tokens and provider API keys must never be copied
--    into a description. `oauth_clients` is intentionally left out: it has
--    no org_id, and audit_log.org_id is NOT NULL.
--
--  * A missing org_id now skips the audit entry instead of raising. Without
--    this, adding a trigger to `profiles` would make signup fail the moment
--    a profile row is created before its org is assigned.
--
-- Everything else here is coverage: goods receipt LINES (receiving disputes
-- are about the lines, and only the header was audited), estimate direct
-- costs / versions / change requests, project direct + subcontract costs,
-- automation sequences and their triggers/conditions (they decide what gets
-- sent to customers), rate matrices, crews, templates and portal access.
-- ─────────────────────────────────────────────────────────────────────────────
      ...
      if v_lead is null then
        ...
        v_description := initcap(replace(v_record_type, '_', ' '))
                         || case when v_record_type in (
                                   ...
                                   'crew','document_template','email_template')
...
DECLARE
  ...
  audited_tables text[] := array[
    -- procurement approvals — who approved what, and who changed the chain
    'approval_flows', 'approval_flow_steps', 'approval_requests',
    -- identity and credentials
    'profiles', 'api_keys', 'integrations', 'oauth_tokens',
    -- finance
    'financial_periods', 'goods_receipt_lines',
    'estimate_direct_costs', 'estimate_versions', 'estimate_change_requests',
    'project_direct_items', 'project_subcontract_costs',
    -- what customers receive
    'crm_automations', 'crm_automation_sequences', 'crm_sequence_triggers',
    'crm_sequence_trigger_conditions', 'crm_sequence_stop_conditions',
    'crm_document_templates', 'crm_email_templates',
    -- pricing and crews
    'crm_service_rate_matrix', 'crm_snow_rate_tiers',
    'crm_crews', 'crm_crew_members',
    -- portal access
    'client_portal_users', 'client_portal_invites'
  ];
BEGIN
...
-- The execution-side sequence tables (crm_sequence_events,
-- crm_sequence_execution_log, crm_sequence_enrollments) are deliberately NOT
-- audited: they are already append-only run logs, and auditing them would
-- duplicate every automation send into audit_log.
