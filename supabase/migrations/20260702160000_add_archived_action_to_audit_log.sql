-- photo_jobs (and projects) toggle is_archived rather than a status column
-- to archive/unarchive a record. fn_audit_log() had no special case for
-- this, so it fell through to the generic per-field diff and always
-- produced a generic "X updated" description/action even when the real
-- action was archiving. Add a dedicated branch, matching the existing
-- status_changed pattern.
...
-- fn_audit_log() now emits 'archived'/'unarchived' for is_archived
-- transitions, but the audit_log.action CHECK constraint didn't allow
-- those values yet — the next real archive/unarchive action would have
-- thrown at the DB level.
