-- fn_audit_log()'s generic "parts quantity adjusted" branch lost its
-- app.suppress_parts_qty_audit check when 20260714000006_wo_costs_audit_trail.sql
-- rewrote the function (the check was correctly present as of
-- 20260710000010_wo_parts_and_unit_cost_audit_attribution.sql, 4 days earlier).
-- Since then every receive_part_quantity()/adjust_part_quantity() call has
-- logged its own attributed audit row AND a duplicate generic "qty X -> Y"
-- row from this trigger. Restoring the suppression check.
