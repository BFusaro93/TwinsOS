-- fn_audit_log()'s generic per-field diff only formats a changed column as
-- dollars when its name ends in _cents, but product_items.price/unit_cost
-- (and parts/pm_schedule_parts/pm_schedule_asset_parts/project_direct_items.
-- unit_cost) are cents columns without that suffix, so their audit entries
-- showed raw integer cents (e.g. "price: 13455 -> 0" instead of "$134.55 ->
-- $0.00"). Also, product_items.cost_layers (an internal JSONB FIFO-cost
-- tracking structure) was being diffed as raw truncated JSON text whenever
-- a receipt changed it alongside quantity_on_hand, producing unreadable
-- entries like 'cost layers: [] -> [{"id": ...'. Widen the dollar-format
-- check to include unit_cost/price by name, and skip cost_layers entirely.
--
      ...
      if (r_old ->> 'deleted_at') is null and (r_new ->> 'deleted_at') is not null then
        ...
        for v_key in select jsonb_object_keys(r_new) loop
          ...
          if (r_old ->> v_key) is distinct from (r_new ->> v_key) then
            if v_key like '%\_cents' or v_key in ('unit_cost', 'price') then
