-- Two audit-trail bugs fixed here:
--
-- 1. crm_invoice_line_items and estimate_line_items never had an audit
--    trigger attached, so adding/editing/removing a line item (including the
--    new per-line discount) produced no audit_log entry at all. These now
--    log against the PARENT invoice/estimate's record_type + record_id, so
--    they show up on the same Audit Trail tab as the rest of that document's
--    history.
--
-- 2. The estimates table's own audit trigger has been mislabeling every
--    entry as record_type='estimates' (the literal table name, via the CASE
--    ELSE fallback) instead of 'estimate', because the CASE statement checked
--    for a table named 'crm_estimates' which has never existed — the real
--    table is 'estimates'. AuditTrailTab queries record_type='estimate', so
--    this has been silently hiding nearly all estimate audit history.
--    Backfilled below.
...
begin
  ...
  -- Resolve the acting user from the live session (shared by every branch below).
  begin
  ...
  -- ── Every other audited table (unchanged from before) ──────────────────────
...
create trigger trg_crm_invoice_line_items_audit
  after insert or update or delete on public.crm_invoice_line_items
  for each row execute function public.fn_audit_log();

create trigger trg_estimate_line_items_audit
  after insert or update or delete on public.estimate_line_items
  for each row execute function public.fn_audit_log();

-- Backfill: correct historical estimate audit entries that were mislabeled
-- 'estimates' (the raw table name) instead of 'estimate' by the old CASE
-- fallback, so they show up on the Audit Trail tab retroactively.
update public.audit_log set record_type = 'estimate' where record_type = 'estimates';
