...
export function resolveMergeTags(
  ...
  return template.replace(/\[(\w+)\]/g, (match) => {
    ...
    // `preserveUnresolvedKnownTags` is for populating an editable draft (no
    // real client/invoice record attached yet, e.g. loading a Documents
    // template into a compose dialog) — leave the tag as literal "[tag]"
    // text so the real send-time resolver still has something to
    // substitute. Without it (the default), a recognized Documents
    // merge-tag name this call just didn't provide a value for degrades to
    // blank so it never ships to a real recipient as literal "[tag]"
    // syntax. Anything NOT a known tag name is always left alone — most
    // likely genuine bracket text the author typed (e.g. "[12 months]"),
