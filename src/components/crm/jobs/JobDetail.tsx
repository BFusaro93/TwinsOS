...
type Tab = "overview" | "services" | "visits" | "notes" | "invoice" | "audit";
...
export function JobDetail({ jobId, initialEditing = false }: Props) {
  ...
  // Auto-create/generate visits on mount when none exist
  useEffect(() => {
    if (!job || autoGeneratedRef.current) return;

    // One-time job: auto-create a single visit from the job's scheduledDate
    if (job.jobType === "one_time" && job.scheduledDate && visits.length === 0) {
    ...
    // Recurring/package: generate up to 365 days of visits
    const isRecurring = job.jobType === "recurring" || job.jobType === "package";
    if (!isRecurring) return;
    const today = new Date().toISOString().slice(0, 10);
    const hasUpcoming = visits.some((v) => v.status === "scheduled" && (v.scheduledDate ?? "") >= today);
    if (!hasUpcoming) {
      autoGeneratedRef.current = true;
      void (async () => {
        try {
          ...
          if (res.ok && data.generated > 0) {
            await qc.invalidateQueries({ queryKey: ['crm-job-visits', 'job', job.id] });
            toast.success(`${data.generated} visit${data.generated !== 1 ? "s" : ""} auto-scheduled`);
          }
        } catch { /* silent */ }
      })();
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex gap-0 border-b bg-white px-6">
        {(["overview", "services", "visits", "notes", "invoice", "audit"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn(
