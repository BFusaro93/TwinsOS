...
<p style="margin:20px 0">[estimatelink]</p>
...
export const DEFAULT_SUBJECT = "Your Estimate from [companyname] — Estimate #[estimatenumber]";
...
interface Props {
  ...
  /** Estimate total (cents) for the [estimatetotal] preview. */
  totalCents?: number;
  /** Estimate date (ISO) for the [estimatedate] preview; defaults to today. */
   ...
   * real [estimatelink] target in Preview. Null → the send route mints one, so the
...
export function SendEstimateDialog({
  ...
  // "estimate") — a richer block-based builder than the old plain
  // subject+body records, and the single place all email templates live.
  const { data: allDocTemplates = [] } = useDocumentTemplates();
  const templates = allDocTemplates.filter((t) => t.docType === "estimate" && t.status === "active");
  ...
  // route's own resolver, same as InvoiceEmailDialog.
  useEffect(() => {
  ...
  // Auto-select the org's default estimate-email document template on open
  useEffect(() => {
  ...
  // [estimatelink] preview: the real live proposal URL when the estimate has
  // one (so Preview can be clicked/copied), otherwise the same button with an
  // explicit "generated on send" note — never a dead href="#".
  const estimateLinkButtonStyle = `color:#fff;background:${brandColor};padding:10px 20px;border-radius:4px;text-decoration:none;font-weight:600;display:inline-block`;
  const estimateLinkPreviewHtml = proposalUrl
    ? `<a href="${proposalUrl}" target="_blank" rel="noopener" style="${estimateLinkButtonStyle}">View Your Proposal →</a>`
    : `<span style="${estimateLinkButtonStyle};cursor:default" title="The proposal link is generated when this email is sent">View Your Proposal →</span>`
  ...
  function previewResolve(text: string) {
    const estimateDateVal = estimateDate ? new Date(estimateDate) : new Date();
