...
export function EmailActivityList() {
  ...
  async function resendSelected() {
    ...
    if (count === 0) return;
    // Resend each via the existing invoice email API where a sentTo exists
    let sent = 0;
    for (const email of selected) {
      ...
      try {
        await fetch("/api/crm/invoices/email", {
          ...
          // The email route expects an invoiceId — resend logs require ref_id on client_activity
          body: JSON.stringify({ invoiceId: email.id }),
        });
        sent++;
      } catch {
        // best-effort
    ...
    toast.success(sent > 0 ? `Resent ${sent} email${sent > 1 ? "s" : ""}` : "No emails could be resent");
    setSelectedIds(new Set());
  }
