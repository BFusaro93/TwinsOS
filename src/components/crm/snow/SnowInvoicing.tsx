...
import {
  useUninvoicedSnowVisits,
  useGenerateSnowInvoices,
  billingGroupKey,
} from "@/lib/hooks/use-snow-invoicing";
import { useSnowRateTiersForJobs } from "@/lib/hooks/use-snow-rate-tiers";
import {
  computeGroupAmountCents,
  describeAmount,
  splitGroupAmountByVisit,
} from "@/lib/snow-billing";
...
export function SnowInvoicing() {
  ...
  const {
    ...
    isError: tiersError,
  } = useSnowRateTiersForJobs(perEventPerInchJobIds);
  ...
  const rows = useMemo(() => {
    ...
    for (const groupVisits of byGroup.values()) {
      ...
      for (const [visitId, amount] of splitGroupAmountByVisit(groupVisits, total)) {
        amountByVisitId.set(visitId, amount);
      }
    }
    return visits.map((v) => ({
      visit: v,
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (result.skippedZeroAmountGroups > 0) {
        toast.warning(
          `${result.skippedZeroAmountGroups} client${result.skippedZeroAmountGroups !== 1 ? "s" : ""} skipped — selected visits priced at $0, check rates`
        );
      }
      if (result.excludedZeroAmountVisits > 0) {
        toast.warning(
