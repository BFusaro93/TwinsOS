...
const STATUS_FILTERS: { label: string; value: PhotoJobStatus | "all" }[] = [
  { label: "All",      value: "all" },
...
export default function PhotoJobsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<PhotoJobStatus | "all">("all");
  const [search, setSearch] = useState("");
  const [showNew, setShowNew] = useState(false);
  const { data: jobs = [], isLoading } = usePhotoJobs(statusFilter);
  const { mutate: createJob, isPending: creating } = useCreatePhotoJob();
  const { data: projects = [] } = useProjects();
  const [form, setForm] = useState({ name: "", customerName: "", address: "", notes: "", projectId: "" });
  ...
  function handleCreate() {
    ...
    createJob(
      { name: form.name, customerName: form.customerName, address: form.address, notes: form.notes || undefined, projectId: form.projectId || undefined },
      { onSuccess: (job) => { setShowNew(false); setForm({ name: "", customerName: "", address: "", notes: "", projectId: "" }); router.push(`/photos/jobs/${job.id}`); } },
    );
  }
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input placeholder="Job site address" value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} />
              </div>
              <div className="sm:col-span-2">
              ...
              <div className="sm:col-span-2">
                <Label className="text-xs">Link to Project for cost tracking (optional)</Label>
                ...
                <p className="mt-1 text-xs text-slate-400">Only link if you want this job tracked in the Projects / cost tab.</p>
              </div>
            </div>
            ...
            {filtered.map((job) => (
              <button key={job.id} className="flex w-full items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-shadow hover:shadow-md" onClick={() => router.push(`/photos/jobs/${job.id}`)}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {job.address && <div className="mt-0.5 flex items-center gap-1 text-xs text-slate-400"><MapPin className="h-3 w-3" /><span className="truncate">{job.address}</span></div>}
                </div>
