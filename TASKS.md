...
- [x] ~~`organizations.twilio_account_sid`/`twilio_messaging_service_sid` exist as per-org override columns, but every org still shares one platform-wide `TWILIO_AUTH_TOKEN` — a real per-tenant Twilio subaccount needs its own auth token too, which is a secret and needs encrypted-at-rest storage~~ — RESOLVED 2026-09-01/09-11: built full per-org Twilio subaccount provisioning ([src/lib/twilio/provisioning.ts](src/lib/twilio/provisioning.ts), [org_sms_registrations](supabase/migrations/20260901120000_org_sms_registrations.sql)) — each org gets its own subaccount, API Key/Secret and Auth Token stored encrypted in Supabase Vault, self-service Trust Hub/A2P 10DLC onboarding UI in Settings → Integrations. Landscapt's own ISV/reseller Twilio account + Trust Hub Primary Customer Profile is live and approved; first real org (sandbox) mid-test as of 2026-09-11.
...
## Shelved — marketing/upsell SMS campaigns (Jobber-style)
Raised 2026-09-11: Jobber lets orgs text promotional/upsell campaigns to clients (e.g. "book your fall cleanup, 10% off"). Not allowed under what's built today — deliberately shelved, not started.

- [ ] The A2P 10DLC campaign this pipeline registers ([submitCampaign()](src/lib/twilio/provisioning.ts)) only discloses transactional/account-notification content in its consent language and sample messages (appointment reminders, crew arrival, job status, invoices). Sending promotional content under that consent would violate the campaign's declared use case — carriers can suspend or throttle the whole number for it, not just reject the next submission. TCPA/CTIA also require marketing consent to be its own separate, explicit opt-in — it can't be implied by or bundled into service-notification consent.
- [ ] To support this legitimately would need: a second, distinct consent checkbox specifically for promotional messages (can't reuse the existing service-notification opt-in), and either an amended campaign (broader disclosed use case + sample messages covering marketing) or a fully separate campaign registration so a marketing-content compliance issue can't take down transactional texts too. Likely also needs its own opt-out handling given CTIA treats marketing STOP requests more strictly.
- [ ] Revisit if/when there's real demand for this from an org — not needed for the current rollout.
...
## Resolved — Vercel plan upgrade for true hourly cron
Found 2026-08-28 building scheduled report emails ([report_schedules](supabase/migrations/20260828225000_report_schedules.sql), [cron route](src/app/api/cron/report-schedules/route.ts)). The account (`bfusaro93s-projects`) was on the Vercel **Hobby** plan, which caps cron jobs at once-per-day regardless of the schedule string — an hourly Vercel Cron entry gets silently throttled, so the per-schedule "send time" picker in the Schedule dialog wouldn't actually fire at the chosen hour. Worked around at the time with GitHub Actions scheduled workflows calling the affected routes directly (`report-schedules-cron.yml` hourly, `sales-meeting-reminders-cron.yml` every 15 min) instead of relying on Vercel's own Cron feature.

- [x] Upgraded to Vercel Pro 2026-09-10. Moved `/api/cron/report-schedules`, `/api/cron/sales-meeting-reminders`, and `/api/crm/sales-meetings/automation-date-triggers` back onto native Vercel Cron entries in `vercel.json` at their real cadence, and deleted both GitHub Actions workflow files.
...
- [ ] Worth deciding whether migrations should be applied via `supabase db push` (filename = version) rather than the MCP, so the ledger and the repo agree.
